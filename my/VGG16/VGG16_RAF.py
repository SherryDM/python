## 说明：1）单纯在目标域(RAF_DB)上训练VGG16分类器，保存模型参数，在目标域(RAF_DB)测试集上测试分类结果:74%
...
dataloader_A = get_loader('/data/dm/data/RAF_DB/train',
                          crop_size=100, image_size=opt.img_size, batch_size=opt.batch_size,
                          num_workers=1)
dataloader_B = get_loader('/data/dm/data/RAF_DB/test',
                          crop_size=100, image_size=opt.img_size, batch_size=opt.batch_size,
                          num_workers=1)
# Optimizers
...
writer.close()###################
# Save the Model
torch.save(classifier.state_dict(), 'VGG16_RAF.pkl')
...
classifier = VGG('VGG16')
classifier.cuda()
classifier.load_state_dict(torch.load('VGG16_RAF.pkl'))
