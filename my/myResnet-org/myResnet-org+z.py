...
dataloader_B = get_loader('/data/dm/data/RAF_DB/train',crop_size=100,
                          image_size=opt.img_size, batch_size=opt.batch_size,num_workers=1)
test_performance = []
dataloader_B_test = get_loader('/data/dm/data/RAF_DB/test',crop_size=100,
                               image_size=opt.img_size, batch_size=opt.batch_size,num_workers=1)
# Optimizers
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        z = Variable(torch.cat([c_org,z],dim=1))
        z = z.cuda()
        ...
        z_rec = Variable(torch.cat([c_trg,z_r],dim=1))
        z_rec = z_rec.cuda()
        ...
        acc = np.mean(np.argmax(label_predFB.data.cpu().numpy(), axis=1) == labels_B.data.cpu().numpy())
        print ("acc",acc)
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images2/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
    ...
    print ("[Epoch %d/%d] [CLF S_acc: %3f, predB_acc: %3f, target_acc: %3f " %
           (epoch, opt.n_epochs,np.mean(s_acc), np.mean(f_B_acc),np.mean(t_acc)))
    ...
    writer.add_scalar('source_performance_acc', S_acc, epoch)
    writer.add_scalar('source_performance_acc_mean', np.mean(source_performance), epoch)
    writer.add_scalar('task_performance_acc', acc, epoch)
    writer.add_scalar('task_performance_acc_mean', np.mean(task_performance), epoch)
    writer.add_scalar('target_acc', target_acc, epoch)
    writer.add_scalar('target_acc_mean', np.mean(target_performance), epoch)
    writer.add_scalar('s_acc_mean', np.mean(s_acc), epoch)
    writer.add_scalar('f_B_acc', np.mean(f_B_acc), epoch)
    writer.add_scalar('t_acc_mean', np.mean(t_acc), epoch)################
writer.close()###################
