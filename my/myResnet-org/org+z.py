...
os.makedirs('images_orgz1', exist_ok=True)
# 定义是否使用GPU
...
parser.add_argument('--lambda_task', type=int, default=0.5, help='lambda_task')
parser.add_argument('--lambda_rec', type=int, default=0.2, help='lambda_rec')
...
########
writer = SummaryWriter('runs_orgz1')
# # model = LeNet()
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images_orgz1/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
    ...
    print ("[Epoch %d/%d] [CLF S_acc: %3f, predB_acc: %3f, target_acc: %3f, testB_acc: %3f]" %
           (epoch, opt.n_epochs,np.mean(s_acc), np.mean(f_B_acc),np.mean(t_acc),np.mean(testB_acc)))
    ...
    writer.add_scalar('test_B', 100*np.mean(testB_acc), epoch)
writer.close()###################
