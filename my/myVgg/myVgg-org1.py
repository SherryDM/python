...
parser.add_argument('--latent_dim', type=int, default=7, help='dimensionality of the noise input')
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        z = Variable(torch.cat([c_org],dim=1))
        z = z.cuda()
        ...
        z_rlabel = Variable(torch.cat([c_org],dim=1))
        z_r = z_rlabel.cuda()
        # z_rec = Variable(torch.cat([z_r, z_rlabel],dim=1))
        # # z_rec = c_org.cuda()
        fake_BA =generator(fake_B,z_r)
        ...
        task_loss_ =    (task_loss(label_predFB, labels_A) + \
        ...
        S_acc = np.mean(np.argmax(classifier(imgs_A).data.gpu().numpy(), axis=1) == labels_A.data.gpu().numpy())
        ...
        FB_acc = np.mean(np.argmax(label_predFB.data.gpu().numpy(), axis=1) == labels_B.data.gpu().numpy())
        # print (FB_acc)
        ...
        target_acc = np.mean(np.argmax(pred_B.data.gpu().numpy(), axis=1) == labels_B.data.gpu().numpy())
        target_performance.append(target_acc)
        if len(target_performance) > 100:
            target_performance.pop(0)

        print ("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [CLF S_acc: %3f%% (%3f%%), predB_acc: %3f%% (%3f%%), target_acc: %3f%% (%3f%%)]" %
               (epoch, opt.n_epochs,
                i, len(dataloader_A),
                d_loss.data[0], g_loss.data[0],
                100*S_acc, 100*np.mean(source_performance),
                100*FB_acc, 100*np.mean(task_performance),
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images_org/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
    for images, labels in dataloader_B_test:
        images = Variable(images.type(FloatTensor))
        labels = Variable(labels.type(LongTensor))
        pred_Btest = classifier(images)
        pred_acc = np.mean(np.argmax(pred_Btest.data.gpu().numpy(), axis=1) == labels.gpu().numpy())
        test_performance.append(pred_acc)
    print('Accuracy of the model on the test images: %f %%' % (100*np.mean(test_performance)))
    testB_acc.append(100*np.mean(test_performance))
...
for images, labels in dataloader_B_test:
    ...
    pred_acc = np.mean(np.argmax(pred.data.gpu().numpy(), axis=1) == labels.gpu().numpy())
    test_performance.append(pred_acc)
print('Accuracy of the model on the test images: %f %%' % (100*np.mean(test_performance)))