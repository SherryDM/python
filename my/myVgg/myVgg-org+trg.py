...
os.makedirs('images_orgtrg', exist_ok=True)
...
########
writer = SummaryWriter('runs_orgtrg')
# # model = LeNet()
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        # Sample noise
        z = Variable(torch.cat([c_org,c_trg],dim=1))
        z = z.cuda()
        ...
        ##
        z_rec = Variable(torch.cat([c_trg,c_org],dim=1))
        z_rec = z_rec.cuda()
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images_orgtrg/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
