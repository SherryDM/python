...
os.makedirs('images_z', exist_ok=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        z_rlabel = Variable(torch.cat([z],dim=1))
        z_r = z_rlabel.cuda()
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images_z/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # save_image(sample, 'images_z/%d-%d.png' % (epoch, i), nrow=int(math.sqrt(batch_size)), normalize=True)
