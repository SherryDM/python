...
os.makedirs('images_trgz0', exist_ok=True)
...
########
writer = SummaryWriter('runs_trgz0')
# # model = LeNet()
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images_trgz0/%d-%d.png' % (epoch, i), nrow=int(math.sqrt(batch_size)), normalize=True)
