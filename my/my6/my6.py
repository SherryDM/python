...
parser.add_argument('--latent_dim', type=int, default=23, help='dimensionality of the noise input')
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        acc = np.mean(np.argmax(label_predB.data.cpu().numpy(), axis=1) == labels_B.data.cpu().numpy())
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images6/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
