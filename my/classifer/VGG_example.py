## 说明：1）在源域(BU3D_FE)上训练VGG16分类器，保存模型参数，在目标域(RAF_DB)测试集上测试分类结果
...
writer.close()###################
# Save the Model
torch.save(classifier.state_dict(), 'VGG16_classifer.pkl')
...
classifier = VGG('VGG16')
classifier.cuda()
classifier.load_state_dict(torch.load('VGG16_classifer.pkl'))
