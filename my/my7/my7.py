...
#       6)标签z=[c_org,c_trg,mask],图像：A,Fake_B,rec_BA,B  不收敛
#       7)标签z=[c_org,mask],图像：A,Fake_B,B,无重构
...
dataloader_A = get_loader('/data/dm/data/BU_3DFE/train',
                          crop_size=900, image_size=opt.img_size, batch_size=opt.batch_size, num_workers=1)
dataloader_B = get_loader('/data/dm/data/RAF_DB/train',
                          crop_size=100, image_size=opt.img_size, batch_size=opt.batch_size,num_workers=1)
# Optimizers
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        z = Variable(torch.cat([c_org,mask_B],dim=1))
        z = z.cuda()
        ...
        ##
        # z_rec = Variable(torch.cat([c_trg, c_org, mask_A],dim=1))
        # z_rec = z_rec.cuda()
        # # z_rec = c_org.cuda()
