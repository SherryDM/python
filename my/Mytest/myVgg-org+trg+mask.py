...
os.makedirs('images_orgtrgm', exist_ok=True)
os.makedirs('images_fakeB', exist_ok=True)
...
parser.add_argument('--latent_dim', type=int, default=16, help='dimensionality of the noise input')
...
generator= Generator(32, 19, 6)
discriminator = Discriminator()
classifier = VGG('VGG16')
print_network(generator, 'G')
...
########
writer = SummaryWriter('runs_orgtrgm')
# # model = LeNet()
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        mask_A = label2onehot(torch.zeros(imgs_A.size(0)), 2)#1,0
        mask_B = label2onehot(torch.ones(imgs_B.size(0)), 2)#0,1
        mask_A = Variable(mask_A)
        mask_A = mask_A.cuda()
        mask_B = Variable(mask_B)
        mask_B = mask_B.cuda()
        # -----------------
        #  Train Generator
        # -----------------
        ...
        # Sample noise
        z = Variable(torch.cat([c_org,c_trg,mask_A],dim=1))
        z = z.cuda()
        # Generate a batch of images
        fake_B = generator(imgs_A, z)
        # save_image(fake_B, 'images_fakeB/%d-%d.png' % (epoch,i), nrow=int(math.sqrt(batch_size)), normalize=True)
        # Perform task on translated source image
        label_predFB = classifier(fake_B)
        ##
        z_rec = Variable(torch.cat([c_trg,c_org,mask_B],dim=1))
        z_rec = z_rec.cuda()
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images_orgtrgm/%d-%d.png' % (epoch,i), nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
    ...
    writer.add_scalar('testB_acc', 100*np.mean(testB_performance), epoch)
writer.close()###################
