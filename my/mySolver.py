from myModel import Generator
from myModel import Discriminator
# from myModel import Classifer
...
class Solver(object):
    ...
    def __init__(self, A_loader, B_loader, config):
        ...
        self.x_hat_dir = config.x_hat_dir
    ...
    def train_multi(self):
        ...
        for i in range(start_iters, self.num_iters):
            for dataset in ['A', 'B']:
                ...
                data_iter = A_iter if dataset == 'A' else B_iter
                ...
                try:
                    ...
                    if dataset == 'A':
                        A_iter = iter(self.A_loader)
                        x_real, label_org = next(A_iter)
                    elif dataset == 'B':
                        B_iter = iter(self.B_loader)
                        x_real, label_org = next(B_iter)
            ...
            ##############################################################################################
                ...
                #
                x_show_list.append(x_reconst)
                x_concat = torch.cat(x_show_list, dim=3)
                sample_path = os.path.join(self.x_hat_dir,'{}-{}x_hat-images.jpg'.format((i+1),dataset))
                save_image(self.denorm(x_concat.data.cpu()), sample_path, nrow=1, padding=0)
                print('Saved x_hat images into {}...'.format(sample_path))
                #
                # Backward and optimize.
            ...
            #####################################################################
                ...
                # # =================================================================================== #
                #
                # if (i+1) % self.n_critic== 0: #self.n_critic=5
                ...
                #     sample_path = os.path.join(self.sample_dir, '{}-x_rec-images.jpg'.format(i+1))
                #     save_image(self.denorm(x_reconst.cpu()), sample_path, nrow=1, padding=0)
                #     print('Saved x_reconst images into {}...'.format(sample_path))
                #     #
                #     # Backward and optimize.
                #     g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_cls * g_loss_cls
                #     self.reset_grad()
                ...
                # if (i+1) % self.log_step== 0: #self.log_step=10
                #     et = time.time() - start_time
                #     et = str(datetime.timedelta(seconds=et))[:-7]
                #     log = "Elapsed [{}], Iteration [{}/{}], Dataset [{}]".format(et, i+1, self.num_iters, dataset)
                #     for tag, value in loss.items():
            ...
            # if (i+1) % self.sample_step == 0: #self.sample_step=100
            #     x_fake_list = [x_fixed]
            #     for c_fixed in c_celeba_list:
            #         c_trg = torch.cat([c_fixed, zero_rafd, mask_celeba], dim=1)
            #         x_fake_list.append(self.G(x_fixed, c_trg))
            ...
            # if (i+1) % self.model_save_step== 0: #self.model_save_step=1000
            ...
            # # Decay learning rates.
            # if (i+1) % self.lr_update_step== 0 and (i+1) > (self.num_iters - self.num_iters_decay):#self.lr_update_step=100
            #     g_lr -= (self.g_lr / float(self.num_iters_decay))
            #     d_lr -= (self.d_lr / float(self.num_iters_decay))
            #     self.update_lr(g_lr, d_lr)
            #     print ('Decayed learning rates, g_lr: {}, d_lr: {}.'.format(g_lr, d_lr))
    ...
    def test_multi(self):
        ...
        for i, (x_real, c_org) in enumerate(self.A_loader):
