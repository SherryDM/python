## 说明：1）单纯在目标域(RAF_DB)上训练VGG16分类器，保存模型参数，在目标域(RAF_DB)测试集上测试分类结果
...
writer.close()###################
# Save the Model
torch.save(classifier.state_dict(), 'Resnet_RAF.pkl')
...
classifier = ResNet18()
classifier.cuda()
classifier.load_state_dict(torch.load('Resnet_RAF.pkl'))
