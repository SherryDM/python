...
#       6)目标域数据加生成的数据训练分类器
#       6)对比试验，z VS [c_org,c_trg,mask]，无重构
...
for epoch in range(opt.n_epochs):
    ...
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        # z_label = Variable(torch.cat([c_org, c_trg, mask_B],dim=1))
        # z = z.cuda()
        ...
        # z_rlabel = Variable(torch.cat([c_trg, c_org, mask_A],dim=1))
        # z_r = z_r.cuda()
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(sample, 'images10/%d.png' % batches_done, nrow=int(math.sqrt(batch_size)), normalize=True)
            # Evaluate performance on translated Domain A
